...
for run_perc in range(10):
    for perc in [0.01, 0.05]:
        for run in range(runs):
            for n_inducing in n_inducings:
                for train_name in train_names:
                    for val_name in val_names:
                        # Save_path
                        save_path = 'experiments/percentage_svgpmix/perc_{}/run_perc_{}/{}/{}_{}/{}/'.format(perc,run_perc,run,train_name,val_name,n_inducing)
                        ...
                        num_samples = int(len(data_loaded.X_train) * perc)

                        # Generate an array of indices from 0 to array_length - 1
                        indices = np.arange(len(data_loaded.X_train))
                        ...
                        # Train dataset
                        X_tr = np.concatenate((data_loaded.X_train[sampled_indices], data_loaded.grx.X_train))
                        z = np.array([[i, x] for i, x in enumerate(np.array(data_loaded.y_train)[sampled_indices])])
                        N_vlc = len(z)
                        ...
                        y_ev = np.concatenate((np.array(data_loaded.y_train)[sampled_indices],data_loaded.grx.train_MV))
